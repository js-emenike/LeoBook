...
async def extract_tab(
    ...
    Two-tier retry strategy:
      - Outer: @aigo_retry(2) reloads the entire page on failure.
      - Inner: 3 attempts of JS re-extraction without page reload.
    """
    ...
    # ── Inner retry loop: re-extract JS up to 3 times WITHOUT page reload ─
    INNER_RETRIES = 3
    last_summary = ""
    ...
    for attempt in range(1, INNER_RETRIES + 1):
        ...
        try:
            ...
            print(f"      [Extract {attempt}/{INNER_RETRIES}] JS failed: {e}")
        ...
        if not matches_raw:
            if scanned_count > 0:
                last_summary = f"[{tab.upper()}] {scanned_count} rows scanned but 0 extracted"
                print(f"      [Extract {attempt}/{INNER_RETRIES}] {last_summary}")
                if attempt < INNER_RETRIES:
                    await asyncio.sleep(1.5)
                continue
        ...
        if passed:
            print(f"    {summary}")
            break  # Contract satisfied — exit inner loop
        else:
            print(f"      [Extract {attempt}/{INNER_RETRIES}] Contract failed: {summary}")
            if attempt < INNER_RETRIES:
                await asyncio.sleep(1.5)  # Brief wait for DOM to settle
    else:
        # All inner attempts exhausted — raise to trigger outer page-reload retry
        raise DataContractViolation(last_summary)
